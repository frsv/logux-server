...
describe('Basic Logux Server specification', () => {
  var test = this
  afterEach(() => {
  ...
  xit('uses arg, env, defaults options in given priority', () => {
    var app = createServer()
  ...
  xit('creates a client on connection', () => {
    ...
    return test.app.listen({ port: uniqPort() }).then(() => {
      ...
      console.info(ws.internalOnOpen)
  ...
  xit('accepts custom HTTP server', () => {
    createReporter(test)
    var port = uniqPort()
